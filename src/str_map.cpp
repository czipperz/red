...
static void insert_unchecked_inner(GenericStrMap::Entry* entry,
                                   size_t size,
                                   cz::Str key,
                                   const void* value) {
...
void GenericStrMap::reserve(cz::AllocInfo info, cz::Allocator allocator, size_t extra) {
...
static GenericStrMap::Entry empty_entry(GenericStrMap* map, cz::Str key, Hash hash) {
    GenericStrMap::Entry entry;
    entry._map = map;
    entry._key = key;
    entry._hash = hash;
...
GenericStrMap::Entry GenericStrMap::find(cz::Str key, Hash key_hash) {
    if (_cap == 0) {
...
void GenericStrMap::Entry::insert_unchecked(size_t size, const void* value) {
    insert_unchecked_inner(this, size, _key, value);
    ++_map->_count;
}

void GenericStrMap::drop(cz::AllocInfo info, cz::Allocator allocator) {
    allocator.dealloc({_masks, mask_alloc_size(_cap)});
    allocator.dealloc({_hashes, sizeof(Hash) * _cap});
    allocator.dealloc({_keys, sizeof(cz::Str) * _cap});
    allocator.dealloc({_values, info.size * _cap});
...
void* GenericStrMap::get_index(cz::AllocInfo info, size_t index) {
    if (mask_is_present(_masks, index)) {
